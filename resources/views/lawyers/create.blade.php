...
<x-card class="p-10 max-w-lg mx-auto mt-24">
    ...
    <form action="/lawyers" method="post" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            ...
            <input type="text" value="{{old('location')}}" class="border border-gray-200 rounded p-2 w-full" name="location" placeholder="Example: 666 Route d'Esch, Belvaux, Luxembourg" />
            @error('location')
            <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea class="border border-gray-200 rounded p-2 w-full" name="languages" rows="6" placeholder="Example: Luxembourg, English, German, French, Dutch, Romanian - type in manually.">{{old('languages')}}</textarea>
            @error('languages')
        ...
        <div class="mb-6">
            ...
            <input type="text" value="{{old('phone')}}" class="border border-gray-200 rounded p-2 w-full" name="phone" placeholder="Example: +352 00 00 00" />
            @error('email')
            <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea class="border border-gray-200 rounded p-2 w-full" name="description" rows="12" placeholder="I'm Narelle Baker a divorce lawyer with 20 years of experience. I did my college in Melbourne at the University of fine lawyers. I'm well versed in international divorce laws with a focus on Luxembourg. etc.">{{old('description')}}</textarea>
            @error('description')
            <p class="text-red-500 text-xs m-2">{{ $message }}</p>
            @enderror
        </div>
